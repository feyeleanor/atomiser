...
func TestReadNumber(t *testing.T) {
	ConfirmReadNumber := func(s string, r interface{}) {
		...
		if x := ReadNumber(scanner); x != r {
			t.Fatalf("ReadNumber(%v) should be %v but is %v", s, r, x)
		}
	}
	...
	RefuteReadNumber := func(s string) {
		...
		defer func() {
			if r := recover(); r == nil {
				t.Fatalf("ReadNumber(%v) should fail but is %v", s, x)
			}
		}()
		x = ReadNumber(NewScanner(strings.NewReader(s)))
	}
